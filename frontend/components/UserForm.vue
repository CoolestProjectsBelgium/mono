<template>
  <FormSection :title="$t('personal_info')">
    ...
    <div v-if="showGuardian" class="mt-6 grid gap-4 md:grid-cols-2">
      ...
      <div>
        ...
        <input id="email_guardian" v-model="model.email_guardian" type="email" class="form-input" :disabled="disabled" />
      </div>
      <div>
        <label class="form-label" for="gsm_guardian">{{ $t('label_mobiel nummer ouders/voogd') }}</label>
        <input id="gsm_guardian" v-model="model.gsm_guardian" class="form-input" :disabled="disabled" />
      </div>
    </div>
  </FormSection>
</template>
