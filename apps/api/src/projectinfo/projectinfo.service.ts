...
export class ProjectinfoService {
  ...
  public async getThumbnail(
    ...
    const attachment = await this.attachmentModel.findOne({
      where: {
        ...
        [Op.or]: [{ internal: false }, { internal: null }],
      },
    });
  ...
  public async getThumbnailAdmin(
    eventId: number,
    attachmentId: number,
  ): Promise<StreamableFile> {
    const attachment = await this.attachmentModel.findOne({
  ...
  private isActiveProject(
    project: Project | null | undefined,
  ): project is Project {
    return Boolean(project) && project!.deletedAt == null;
  }

  private async countRegisteredCoParticipants(
    projectId: number,
  ): Promise<number> {
  ...
  private async buildParticipants(
    ...
    for (const membership of registeredMemberships) {
      ...
      registered.push({
        id: participant.id,
        name: this.formatParticipantName(
  ...
  public async getProjectInfo(userId: number): Promise<OwnProjectDto> {
    ...
    const registeredCoParticipants = await this.countRegisteredCoParticipants(
      project.id,
    );
  ...
  public async createProject(
    userId: number,
    createProjectDto: OwnProjectDto,
  ...
  public async updateProject(
    userId: number,
    updateProjectDto: OwnProjectDto,
  ): Promise<OwnProjectDto> {
    this.logger.log(
  ...
  public async deleteUnusedVoucher(
    userId: number,
    voucherGuid: string,
  ): Promise<void> {
    const userProject = await this.userProjectModel.findOne({
  ...
  public async changeProjectOwner(
    userId: number,
    newOwnerId: number,
  ): Promise<void> {
    const userProject = await this.userProjectModel.findOne({
