...
export class VotingService {
  ...
  async submitVotes(
    ...
    await this.voteModel.bulkCreate(
      votes.map((v) => ({
        categoryId: v.id,
        projectId,
        accountId,
  ...
  private async fillAwardRecords(
    eventId: number,
    projects: Project[],
  ): Promise<Award[]> {
    await this.awardModel.destroy({ where: { eventId } });
  ...
  private async assignAwardWinners(
    ...
    for (const [categoryId, categoryResults] of byCategory) {
      const candidates = categoryResults
        ...
        .sort(
          (first, second) =>
            second.adjusted_average_percent - first.adjusted_average_percent,
        )
        .map((result, index) => ({
          projectId: result.projectId,
          projectName: awards.find(
            (award) => award.projectId === result.projectId,
          )!.project.name,
      ...
      const winner = candidates.find(
        (candidate) => !usedProjectIds.has(candidate.projectId),
      );
      if (!winner) continue;

      usedProjectIds.add(winner.projectId);
      const award = awards.find(
        (award) => award.projectId === winner.projectId,
      );
      if (!award) continue;
  ...
  async getAwardAssignments(eventId: number): Promise<AwardAssignmentDto[]> {
    ...
    const awards = await this.awardModel.findAll({
      where: { eventId },
      include: [Project],
    });
      ...
      const candidates = categoryResults.map((result) => ({
        ...
        rank:
          categoryResults.findIndex(
            (candidate) => candidate.categoryId === result.categoryId,
          ) + 1,
        adjustedAveragePercent: Number(result.adjusted_average_percent),
        medianPercent: Number(result.median_percent),
        minPercent: Number((result as any).min_percent ?? 0),
  ...
  private async calculateVotesByCategory(
    eventId: number,
  ): Promise<Map<number, VotesCalculationDto[]>> {
    const results = await this.calculateVotes(eventId);
