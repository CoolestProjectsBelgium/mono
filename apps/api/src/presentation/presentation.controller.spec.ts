...
describe('PresentationController', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    controller = new PresentationController(presentationService as unknown as PresentationService);
  });
  ...
  describe('getSlideImage', () => {
    it('sets ETag/Last-Modified and returns the file on a normal request', async () => {
      ...
      presentationService.getSlideImage.mockResolvedValue({ file, hash: 'abc123', generatedAt });
  ...
  describe('getSlideMeta (HEAD)', () => {
    it('sets ETag/Last-Modified without calling the image-generating path', async () => {
      const generatedAt = new Date('2026-01-01T00:00:00.000Z');
      presentationService.getSlideMeta.mockResolvedValue({ hash: 'abc123', generatedAt });
      const res = fakeResponse();

      await controller.getSlideMeta({ currentEvent: 1 } as never, 'slide-1', res);
    ...
    it('omits Last-Modified when the slide has never been generated', async () => {
      presentationService.getSlideMeta.mockResolvedValue({ hash: 'abc123', generatedAt: null });
      const res = fakeResponse();

      await controller.getSlideMeta({ currentEvent: 1 } as never, 'slide-1', res);

      expect(res.setHeader).toHaveBeenCalledWith('ETag', '"abc123"');
      expect(res.setHeader).not.toHaveBeenCalledWith('Last-Modified', expect.anything());
    });
  });
});
