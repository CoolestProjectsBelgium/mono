...
export interface RawSqlColumn {
    path: string
    type?: 'string' | 'float' | 'number' | 'boolean' | 'date' | 'datetime' | 'mixed' | 'textarea'
    isId?: boolean
}
...
const buildWhere = (filter: unknown, baseReplacements: Record<string, unknown>) => {
    const clauses: string[] = []
    const replacements: Record<string, unknown> = { ...baseReplacements }
    const elements = Object.values((filter as { filters?: Record<string, FilterElement> })?.filters ?? {})
...
export class RawSqlResource extends BaseResource {
    ...
    properties(): BaseProperty[] {
        return this.cfg.columns.map((column, position) => new BaseProperty({
            path: column.path,
    ...
    property(path: string): BaseProperty | null {
        return this.properties().find((property) => property.path() === path) ?? null
    }
    ...
    async count(filter: unknown): Promise<number> {
        const { where, replacements } = buildWhere(filter, this.cfg.replacements ?? {})
    ...
    async find(
        ...
        const sortBy = options.sort?.sortBy && knownColumns.has(options.sort.sortBy)
            ? options.sort.sortBy
            : this.cfg.primaryKey
        ...
        const rows = await sequelize.query(
            `SELECT * FROM ${this.derivedTable()} ${where} ORDER BY \`${sortBy}\` ${direction} LIMIT :limit OFFSET :offset`,
            {
                ...
                replacements: { ...replacements, limit: options.limit ?? 20, offset: options.offset ?? 0 },
            },
        )

        return rows.map((row) => new BaseRecord(row as Record<string, unknown>, this))
    }

    async findOne(id: string): Promise<BaseRecord | null> {
        const rows = await sequelize.query(
            `SELECT * FROM ${this.derivedTable()} WHERE \`${this.cfg.primaryKey}\` = :id`,
            { type: QueryTypes.SELECT, replacements: { ...(this.cfg.replacements ?? {}), id } },
        )

        return rows[0] ? new BaseRecord(rows[0] as Record<string, unknown>, this) : null
    }
    ...
    async findMany(ids: Array<string | number>): Promise<BaseRecord[]> {
        ...
        const rows = await sequelize.query(
            ...
            { type: QueryTypes.SELECT, replacements: { ...(this.cfg.replacements ?? {}), ids } },
        )
