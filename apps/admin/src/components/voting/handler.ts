...
export const Handler = async (
  ...
  _response: any,
  context: any,
): Promise<VotingOverview | { success: true }> => {
  ...
  if (request.method?.toLowerCase() === 'post') {
    ...
    if (action === 'start') {
      ...
      await api.post('/admin/voting/message', {
  ...
  const votesByProjectCategory = Array.from(
    projectCategories,
    ([project, categories]) => ({
      project,
      ...categories,
  ...
  const sortedVoteIntervals = Array.from(timeTotals, ([date, votes]) => ({
    date,
    votes,
  ...
  if (!status.votingOpen) {
    try {
      const calculated = (
        await api.get<Array<Omit<VotingResult, 'projectName'>>>(
          '/admin/voting/results',
        )
      ).data;
      const projectIds = [
        ...new Set(calculated.map((result) => result.projectId)),
      ];
      const projects = await Project.findAll({
        where: { eventId, id: { [Op.in]: projectIds } },
        attributes: ['id', 'name'],
      });
      ...
      results = calculated.map((result) => ({
        ...result,
        projectName:
          projectNames.get(result.projectId) ?? `Project #${result.projectId}`,
      }));
    } catch (error) {
  ...
  const awardCategories: AwardCategoryOption[] = awardCategoryRecords.map(
    (category) => ({
      id: category.id,
      name: category.name,
    }),
