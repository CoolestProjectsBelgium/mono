        ...
        <Box textAlign="center" color="white">
          ...
          <Text>
            ...
            {data.officialStartDate !== undefined
              ? new Intl.DateTimeFormat('en-BE', options).format(
                  new Date(data.officialStartDate),
                )
              : 'No event'}
          </Text>
        ...
        <Box width={[1, 1, 1 / 2]} p="lg">
          <Card as="a" flex>
            <Box ml="xl">
              ...
              <ul>
                <li>
                  {data.total_projects ?? 0}/{data.maxRegistration ?? 0}{' '}
                  Projects Remaining / with {data.total_usedVouchers ?? 0}{' '}
                  Co-Worker(s)
                </li>
                <li>
                  {(data.total_users || 0) -
                    (data.total_usedVouchers || 0) -
                    (data.total_projects || 0)}{' '}
                  user(s) without Project
                </li>
                <li>
                  {data.total_videos ?? 0} Project(s) with foto/video confirmed
                </li>
              </ul>
            </Box>
          </Card>
