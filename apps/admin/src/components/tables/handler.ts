...
export const Handler = async (
  ...
  if (request.method?.toLowerCase() === 'post') {
    ...
    if (payload.action === 'swap-tables') {
      ...
      if (
        tableIds.length !== 2 ||
        tableIds.some((id) => !Number.isInteger(id))
      ) {
        throw new Error('Select exactly two tables to swap');
      }

      const tables = await EventTable.findAll({
        where: { id: tableIds, eventId },
      });
      if (tables.length !== 2) {
      ...
      if (projectId !== null) {
        const project = await Project.findOne({
          where: { id: projectId, eventId, deletedAt: null },
        });
        if (!project) {
