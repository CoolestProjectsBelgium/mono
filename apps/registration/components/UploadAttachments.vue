  ...
  <FormSection>
    <div class="space-y-4">
      <p
        v-if="limitReached"
        class="text-gray-600"
        data-testid="upload-limit-reached"
      >
        {{ limitReachedMessage }}
      </p>
      <FormField
      ...
      >
        <template
          #default="{ inputId, inputClass, ariaInvalid, ariaDescribedby }"
        >
          <input
            :id="inputId"
            type="file"
      ...
      <ApiUnavailableBanner
        v-if="unavailable"
        message-key="apiUnavailable.attachments"
      />
    </div>
  </FormSection>
</template>
...
function uploadErrorMessage(
  code: 'tooLarge' | 'invalidType' | 'tooMany',
): string {
  if (code === 'invalidType') {
  ...
  return t('validation_uploadTooLarge', {
    maxSize: formatFileSize(props.maxUploadSize),
  });
}
...
async function onFileSelect(event: Event) {
  ...
  try {
    const result = await uploadFile(file, {
      ...
      onPhase: (phase) => {
        converting.value = phase === 'converting';
      },
    });
    if (!result.ok) {
      if (
        result.code === 'tooLarge' ||
        result.code === 'invalidType' ||
        result.code === 'tooMany'
      ) {
