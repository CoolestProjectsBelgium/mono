...
describe('attachment-normalize', () => {
  it('classifies native files', () => {
    expect(classifyUploadFile(new File(['x'], 'photo.png', { type: 'image/png' }))).toBe('native')
    expect(classifyUploadFile(new File(['x'], 'clip.mp4', { type: 'video/mp4' }))).toBe('native')
  })
  ...
  it('classifies convertible videos', () => {
    expect(classifyUploadFile(new File(['x'], 'clip.mov', { type: 'video/quicktime' }))).toBe('convertible-video')
  })
  ...
  it('normalizes webp via canvas', async () => {
    ...
    expect(result.needsServerNormalize).toBe(false)
  })
})
