    ...
    <template v-if="isProjectOwner" data-testid="project-owner-view">
      <form class="mt-6" @submit.prevent="onSave" @keydown.enter="onFormKeydown">
        ...
        <div class="mt-6 flex gap-4">
          ...
          <NuxtLink :to="localePath('/upload')" class="btn-primary" data-testid="photos-link">{{ $t('upload.photosCta') }}</NuxtLink>
...
async function onLeaveProject() {
  ...
  const selfParticipant = coworkerParticipants.value.find(participant => participant.self)
  if (!selfParticipant?.token) {
  ...
  try {
    await leaveProject(selfParticipant.id, selfParticipant.token)
