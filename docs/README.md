# Documentation hub

Source of truth for Coolest Projects monorepo architecture and per-package orientation.

**Agents:** start at [AGENTS.md](../AGENTS.md), then open the page for the area you are editing.

## Cross-cutting

- [architecture.md](architecture.md) — monorepo map, dependencies, key flows
- [local-setup.md](local-setup.md) — Dev Container and local URLs
- [build-tools.md](build-tools.md) — Level27 publish CLI (`build_tools/`)
- [email-templates/](email-templates/README.md) — canonical copy for core participant emails (en/nl/fr)
- [unused-code.md](unused-code.md) — unused-code audit punch list

## Apps

| App | Doc | Status |
|-----|-----|--------|
| API | [apps/api.md](apps/api.md) | deep |
| Admin | [apps/admin.md](apps/admin.md) | deep |
| Voting | [apps/voting.md](apps/voting.md) | deep |
| Registration | [apps/registration.md](apps/registration.md) | deep |
| Event guide | [apps/eventguide.md](apps/eventguide.md) | deep |
| CDJ web INT gallery | [apps/cdj-web-int.md](apps/cdj-web-int.md) | stub |
| Presentation | [apps/presentation.md](apps/presentation.md) | deep |

## Packages

| Package | Doc | Status |
|---------|-----|--------|
| Database | [packages/database.md](packages/database.md) | deep |
