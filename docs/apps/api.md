...
**Hashing stays cheap.** A slide's hash is `sha256` of its `body` + `imagePath` + the exact (DB-only) data feeding it — computing every slide's hash for the list endpoint never touches Puppeteer or the filesystem. It's **resolution-independent** (render size doesn't change content), so a content edit invalidates every cached resolution for that slide at once. `PresentationRender` is the render-cache index — one row per `(slideKey, resolution)` pair, `contentHash` + `imagePath` + `generatedAt` — a request only renders when the freshly computed hash doesn't match the cached one for that resolution. `HEAD /presentation/:key` deliberately isn't left to Express's auto-derived `HEAD`-from-`GET` (which would run the full render-if-stale path) — it calls a hash-only lookup that never renders.

**Multiple render resolutions, from a fixed list.** `GET /presentation`, `GET /presentation/:key`, and `HEAD /presentation/:key` all take an optional `?resolution=WIDTHxHEIGHT` query param, validated against a short, hardcoded allowlist (`PRESENTATION_RESOLUTIONS` in `presentation-resolutions.ts`; currently `1280x720`/`1920x1080`/`2560x1440`/`3840x2160`, all 16:9) — an unsupported value is a `400`. Omitted → defaults to `1920x1080`, so a Pi (or the admin preview) that never sends it keeps working unchanged. Every slide `body` is still authored (and every already-saved one in the DB was written) against one canonical `1920x1080` coordinate space — rendering at another resolution wraps the content in a container fixed at that canonical size and applies a uniform `transform: scale()` to fit the real, target-sized viewport/`<body>`, so **admins never author per-resolution content**. The background image (`imagePath`/presentation asset) stays on the outer, target-sized `<body>` with `background-size: cover`, which already scales correctly per resolution on its own. `GET /presentation/resolutions` → `{ resolutions: [{key, width, height}], default }` lets a client discover the allowed list; declared before `GET /presentation/:key` in the controller so the literal path isn't shadowed by the dynamic route.
...
- `GET /presentation/resolutions` → the allowed render resolutions + default, see above.
...
None of these pass a `resolution` — the admin preview always renders at the canonical `1920x1080`, regardless of what resolutions individual Pis are configured for.
