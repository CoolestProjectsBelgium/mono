...
The mail context's `year` comes from `eventYear(event)`, which computes it via `Intl.DateTimeFormat` against the `DISPLAY_TIMEZONE` env var (`mail-context.ts`'s `displayTimezone()`, default `'Europe/Brussels'`) rather than `.getFullYear()` — deterministic regardless of the server process's own OS timezone, and the same env var `apps/admin` reads for its Event date display (`build_tools/env/*.env.example` sets it identically in both apps; not shared via `@coolestprojects/database`, which is schema, not app config). `presentation.service.ts`'s slide-deck rendering reuses the same `eventYear` rather than duplicating the computation.
