...
Staff upload Visio SVG exports via Admin → **Floor plans**; the AdminJS handler proxies to `POST /admin/floorplans` on the API, which auto-processes into `table_XX` groups and stores under `UPLOAD_ROOT/floorplans/` on the **API server** (not the admin host). Clickable table polygons are measured from each `table_XX` group's geometry in SVG viewBox units (text labels are ignored; Visio `width`/`height` in inches is not used for hit-testing).
