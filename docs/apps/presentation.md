...
There's no dedicated display app yet (see **Out of scope** below), but `sync-deck.sh` has first-class support for driving [feh](https://feh.finalrewind.org/) (X11) or [imv](https://sr.ht/~exec64/imv/) (native Wayland) as that client, since a plain directory + default sort can't reproduce the deck: slide keys (`slide-11`, `slide-3`, …) don't sort into the real `order`, and both tools only have one global slideshow delay, not a per-slide one.
