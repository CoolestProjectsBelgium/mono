...
| `Event` | Not event-scoped by a foreign key — it's the event itself. `super_admin` sees and can create/edit/delete every event; every other role only sees the event tied to their session (`id` match against `currentAdmin.eventId`) and gets read-only access (`show` only, no `new`/`edit`/`delete`). Its 8 date fields (`eventBeginDate`, `registrationOpenDate`, `registrationClosedDate`, `projectClosedDate`, `officialStartDate`, `eventEndDate`, `votingStartDate`, `votingEndDate`) render show/list in the `DISPLAY_TIMEZONE` env var's zone with the zone labeled (`components/timezone/`) — the same env var `apps/api`'s `mail-context.ts` reads, so a date reads identically in both apps; edit stays the native browser-local `datetime-local` input. Lives in the **System** navigation group. |
