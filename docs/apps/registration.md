...
2. **Registration** — catalogs (`/tshirts`, `/questions`, `/approvals`, `/dojos`) + `POST /registration`; optional affiliation (`via_type` + `via`) for CoderDojo, another organisation, or taking part individually. Dojo and postal-code pickers are comboboxes: mouse selects on press; touch selects on finger-up if the pointer barely moved (so a scroll does not count as a pick). Blur does not close the list (iOS blurs the input before the tap reaches an option); it stays open until a pick, Escape, or a press outside. Mobile fields (`gsm`, `gsm_guardian`) accept spaces and separators (`0470 12 34 56`, `+32 470 12 34 56`) and are stored compact so they fit `STRING(13)`.
