...
describe('Attachments (e2e)', () => {
  ...
  let attachmentModel: typeof Attachment;
  ...
  beforeAll(async () => {
    ...
    attachmentModel = moduleFixture.get(getModelToken(Attachment));
  ...
  it('creates attachment, lists it, refreshes SAS, and deletes it', async () => {
    ...
    expect(createResponse.body.url).toContain(blob.blob_name);

    const sasResponse = await request(app.getHttpServer())
      .post(`/attachments/${blob.blob_name}/sas`)
      .set('Authorization', token);

    expect(sasResponse.status).toBe(201);
    expect(sasResponse.body.url).toContain(blob.blob_name);
    ...
    expect(projectinfoResponse.body.attachments[0].url).toContain(
      blob.blob_name,
    );

    const deleteResponse = await request(app.getHttpServer())
      .delete(`/attachments/${blob.blob_name}`)
      .set('Authorization', token);
